...
| LLM (summaries) | OpenAI `gpt-5.6-luna` |
| LLM (RAG reports) | OpenAI `gpt-5.6-terra` |
...
| Multi-Agent Pipeline | CrewAI — four-agent crew (Researcher: gpt-5.6-luna, Brief Critic: gpt-5.6-luna, Writer: gpt-5.6-terra, Report Critic: gpt-5.6-luna) with two conditional revision loops (brief critic → Researcher, report critic → Writer) — see `backend/agents/` |
...
| `OPENAI_API_KEY` | Model inference LLM calls (`gpt-5.6-luna`, `gpt-5.6-terra`, embeddings) + CrewAI agents |
...
| `POST /ai/summarise-rag-crew` | 5/minute | Four-agent CrewAI pipeline — expensive `gpt-5.6-terra` calls |
...
│   │   ├── llms.py               # researcher_llm (gpt-5.6-luna), writer_llm (gpt-5.6-terra), critic_llm (gpt-5.6-luna, temp=0)
│   │   ├── agents/
