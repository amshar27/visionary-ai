...
- **Supabase service-role key, no RLS.** `backend/db.py` uses `SUPABASE_SERVICE_ROLE_KEY`, which **bypasses Row Level Security** entirely. No RLS policies were added. Every DB read/write runs with full privileges, so the API layer is the *only* thing standing between a request and the whole database — and per the point above, that layer does not authenticate callers.
...
- **Uploads are not scanned or re-encoded.** Magic-byte validation confirms a file *starts* like a JPEG/PNG; it does not prove the rest is well-formed, and no malware scanning or image re-encoding is performed. Files land in a **public** Supabase bucket, so retinal scan URLs are effectively public if guessed or leaked.
...
- `GET /screenings/sessions/{session_id}/mc-pdf` — downloads the already-saved MC PDF (latest `mc_certificates.mc_url`, fetched via `requests.get`) as a `StreamingResponse` (`application/pdf`, `attachment; filename="mc_{session_id}.pdf"`). 404 if no MC; 502 if the download fails. Mirrors `report-pdf`. Backs the doctor "Export MC as PDF" button on view-only sessions (shown only when an MC exists).
